...
//   friendAvatars: Map<string, string>;
...
//   setFriendAvatar: (friendId: string, imageUri: string) => Promise<{ error: Error | null }>;
//   updatePublicAvatar: (imageUri: string) => Promise<{ error: Error | null }>;
...
//   // FIX: Use the stable user ID for dependencies instead of the whole user object
...
//     if (!userId) return;
//     const { data, error } = await supabase.rpc('get_personal_perfect_streak', { p_user_id: userId });
...
//     if (!userId) return;
//     const { data: friendships, error: friendsError } = await supabase.from('friendships').select(`*, requester:requester_id(id, name, email, gender, is_private, avatar_url), addressee:addressee_id(id, name, email, gender, is_private, avatar_url)`).or(`requester_id.eq.${userId},addressee_id.eq.${userId}`);
...
//   const loadFriendAvatars = useCallback(async () => {
//     if (!userId) return;
//     const { data, error } = await supabase
//       .from('friend_avatars')
//       .select('friend_id, avatar_url')
//       .eq('user_id', userId);

//     if (error) {
//       console.error("Error loading friend avatars:", error);
//     } else {
//       const newAvatarsMap = new Map<string, string>();
//       data.forEach(avatar => {
//         if (avatar.avatar_url) {
//             newAvatarsMap.set(avatar.friend_id, avatar.avatar_url);
//         }
//       });
//       setFriendAvatars(newAvatarsMap);
//     }
//   }, [userId]);
...
//     ]);
//     setLoading(false);
//   }, [userId, loadProfile, loadPersonalStreak, loadFriendsAndStreaks, loadLeaderboardData, loadAppSettings, loadCurrentExemption, loadFriendAvatars]);
...
//       .on('postgres_changes', { event: '*', schema: 'public', table: 'exemption_periods', filter: `user_id=eq.${userId}` }, handleAllUpdates)
//       .subscribe();

//     const friendAvatarsListener = supabase
//       .channel('public:friend_avatars')
//       .on('postgres_changes', { event: '*', schema: 'public', table: 'friend_avatars', filter: `user_id=eq.${userId}` }, loadFriendAvatars)
//       .subscribe();
...
//       supabase.removeChannel(friendAvatarsListener);
//     };
//   }, [userId, loadPersonalStreak, loadFriendsAndStreaks, loadLeaderboardData, loadAppSettings, loadCurrentExemption, loadFriendAvatars]);
...
//       if(!user) return; // Added safety check for user
...
//   const setFriendAvatar = async (friendId: string, imageUri: string) => {
...
//             contentType = imageUri.match(/data:(image\/[^;]+);/)?.[1];
...
//         const fileName = `${user.id}_${friendId}.${fileExt}`;
...
//         setFriendAvatars(prev => new Map(prev).set(friendId, cacheBustedUrl));

//         return { error: null };
//     } catch (error) {
//         console.error("Failed to set friend avatar:", error);
//         return { error: error as Error };
//     }
//   };

//   const updatePublicAvatar = async (imageUri: string) => {
//     if (!user) return { error: new Error("User not logged in.") };
...
//       return { error: null };
//     } catch (error) {
//       console.error("Failed to update public avatar:", error);
//       return { error: error as Error };
//     }
//   };
...
//       currentExemption, isExemptedToday, friendAvatars,
//       updateProfile, sendFriendRequest, respondToFriendRequest,
//       togglePrayerAccess, removeFriend, forceReloadLeaderboard,
//       updateAppSetting, startExemption, endExemption,fetchData, setFriendAvatar, updatePublicAvatar,
//     }}>
//       {children}
...
export function SupabaseUserProvider({ children }: { children: React.ReactNode }) {
  ...
  // FIX: Use the stable user ID string for all dependencies to prevent loops.
  const userId = user?.id;
  ...
  const loadLeaderboardData = useCallback(async () => {
    if (!userId) return;
    // Assuming you have a leaderboard function that needs p_user_id
    const { data, error } = await supabase.rpc('get_friends_leaderboard', { p_user_id: userId });
    if (error) console.error('Error fetching leaderboard data:', error);
    else setLeaderboard(data || []);
  }, [userId]);
