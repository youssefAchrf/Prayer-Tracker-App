...
//   const loadFriendsAndStreaks = useCallback(async () => {
//     if (!user) return;
//     const { data: friendships, error: friendsError } = await supabase.from('friendships').select(`*, requester:requester_id(id, name, email, gender, is_private, avatar_url), addressee:addressee_id(id, name, email, gender, is_private, avatar_url)`).or(`requester_id.eq.${user.id},addressee_id.eq.${user.id}`);
//     if (friendsError) console.error('Error loading friends:', friendsError);
//     else {
//       const typedFriendships: Friendship[] = friendships?.map(f => ({
//         ...f,
...
//     ]);
//     setLoading(false);
//   }, [user, loadProfile, loadPersonalStreak, loadFriendsAndStreaks, loadLeaderboardData, loadAppSettings, loadCurrentExemption, loadFriendAvatars]);
...
//       .on('postgres_changes', { event: '*', schema: 'public', table: 'friend_avatars', filter: `user_id=eq.${user.id}` }, loadFriendAvatars)
//       .subscribe();
...
//     };
//   }, [user, loadPersonalStreak, loadFriendsAndStreaks, loadLeaderboardData, loadAppSettings, loadCurrentExemption, loadFriendAvatars]);
...
//         // --- START OF FIX ---
//         // 1. Create a new URL with a unique timestamp to "bust" the cache.
//         const cacheBustedUrl = `${publicUrl}?t=${new Date().getTime()}`;

//         // 2. Save the new, unique URL to the database.
//         const { error: dbError } = await supabase
//             .from('friend_avatars')
//             .upsert({
//                 user_id: user.id,
//                 friend_id: friendId,
//                 avatar_url: cacheBustedUrl, // Use the new cache-busted URL
//             }, { onConflict: 'user_id, friend_id' });
//         // --- END OF FIX ---

//         if (dbError) throw dbError;

//         // Update the local state with the new URL so the UI updates instantly
...
//       updateProfile, sendFriendRequest, respondToFriendRequest,
//       togglePrayerAccess, removeFriend, forceReloadLeaderboard,
//       updateAppSetting, startExemption, endExemption,fetchData, setFriendAvatar
//     }}>
//       {children}
//     </SupabaseUserContext.Provider>
//   );
...
interface SupabaseUserContextType {
  ...
  endExemption: (exemptionId: string) => Promise<void>;
  setFriendAvatar: (friendId: string, imageUri: string) => Promise<{ error: Error | null }>;
  updatePublicAvatar: (imageUri: string) => Promise<{ error: Error | null }>; // Add this line
  fetchData: () => Promise<void>;
}
...
export function SupabaseUserProvider({ children }: { children: React.ReactNode }) {
  ...
  const updatePublicAvatar = async (imageUri: string) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage
          .from('avatars') // Use the new 'avatars' bucket
          .upload(filePath, blob, {
              cacheControl: '3600',
              upsert: true,
              contentType: contentType,
      ...
      // Update the user's public avatar_url in the 'users' table
      const { error: dbError } = await supabase
          .from('users')
